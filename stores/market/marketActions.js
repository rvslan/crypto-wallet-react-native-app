...
export function getHoldings(
  ...
  return (dispatch) => {
    ...
    let ids = holdings.map((holding) => holding.id).join(',');
      ...
      .then((response) => {
        if (response.status == 200) {
          let myHoldings = response.data.map((item) => {
            // Retrive our current holding
            let coin = holdings.find((holding) => item.id === holding.id);
            ...
            return {
              ...
              qty: coin.qty, // Quantity from our holdings
              total: coin.qty * item.current_price, // Total from our holdings
